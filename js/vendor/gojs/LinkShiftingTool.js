...
LinkShiftingTool.prototype.updateAdornments = function (part) {
    ...
    if (link.isSelected && !this.diagram.isReadOnly) {
        ...
        if (selelt !== null && link.actualBounds.isReal() && link.isVisible() &&
            selelt.actualBounds.isReal() && selelt.isVisibleObject()) {
            let spot = link.computeSpot(false);
            if (spot.isSide() || spot.isSpot()) {
                adornment = link.findAdornment(category);
                if (adornment === null) {
...
LinkShiftingTool.prototype.makeAdornment = function (selelt, toend) {
    let adornment = new go.Adornment();
    adornment.type = go.Panel.Link;
    let h = (toend ? this.toHandleArchetype : this.fromHandleArchetype);
    if (h !== null) {
...
LinkShiftingTool.prototype.doActivate = function () {
    ...
    if (h === null) return;
    let ad = h.part;
    let link = ad.adornedObject.part;
...
LinkShiftingTool.prototype.doReshape = function (pt) {
    let ad = this._handle.part;
    let link = ad.adornedObject.part;
    let fromend = ad.category === "LinkShiftingFrom";
    let port = null;
    if (fromend) {
        port = link.fromPort;
    } else {
    ...
    let portb = new go.Rect(port.getDocumentPoint(go.Spot.TopLeft),
        port.getDocumentPoint(go.Spot.BottomRight));
    let lp = link.getLinkPointFromPoint(port.part, port, port.getDocumentPoint(go.Spot.Center), pt, fromend);
    let spot = new go.Spot((lp.x - portb.x) / (portb.width || 1), (lp.y - portb.y) / (portb.height || 1));
    if (fromend) {
        link.fromSpot = spot;
    } else {
        link.toSpot = spot;
